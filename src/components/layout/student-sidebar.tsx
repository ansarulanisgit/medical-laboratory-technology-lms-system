...
export function StudentSidebar() {
  ...
  const navItems = React.useMemo(() => {
    // Core shared modules available across all roles
    const sharedModules = [
    ...
    if (currentRole === "ADMIN") {
      return [
        sharedModules[0],
        { href: "/student/users", label: "User Directory", icon: Users, exact: false },
        ...sharedModules.slice(1),
      ];
    }

    // MENTOR and STUDENT have the full standardized shared modules
  ...
  return (
    ...
    >
      ...
      {/* Role-adaptive Navigation List with hidden scrollbar to prevent offset */}
      <div
        onScroll={() => setTooltip(null)}
        className={cn(
          "flex-1 overflow-y-auto overflow-x-hidden py-3 space-y-1 transition-all duration-300 scrollbar-none no-scrollbar [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden",
      ...
      {isCollapsed && tooltip && (
        <div
          className="fixed left-[78px] z-50 pointer-events-none -translate-y-1/2 px-3.5 py-1.5 rounded-xl bg-primary text-primary-foreground text-xs font-semibold shadow-lg shadow-emerald-950/25 whitespace-nowrap animate-in fade-in-0 zoom-in-95 duration-150 flex items-center gap-2 border border-emerald-400/30"
          style={{ top: tooltip.top + 'px' }}
        >
          <div className="absolute -left-1 top-1/2 -translate-y-1/2 w-2 h-2 rotate-45 bg-primary border-l border-b border-emerald-400/30" />
          <span className="relative z-10">{tooltip.label}</span>
          {tooltip.isActive && (
            <span className="relative z-10 h-1.5 w-1.5 rounded-full bg-white/90 shadow-2xs shrink-0" />
          )}
        </div>
      )}
    </aside>
